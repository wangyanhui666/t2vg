...
class FirUpsample2D(nn.Module):
    def __init__(self, channels=None, out_channels=None, use_conv=False, fir_kernel=(1, 3, 3, 1)):
        ...
        if use_conv:
            self.Conv2d_0 = Pseudo3DConv(channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.use_conv = use_conv
    ...
    def forward(self, hidden_states):
        if self.use_conv:
            ...
            height = height + self.Conv2d_0.spatial_conv.bias.reshape(1, -1, 1, 1)
        else:
...
class FirDownsample2D(nn.Module):
    ...
    def forward(self, hidden_states):
        if self.use_conv:
            ...
            hidden_states = downsample_input + self.Conv2d_0.spatial_conv.bias.reshape(1, -1, 1, 1)
        else:
...
def upfirdn2d_native(tensor, kernel, up=1, down=1, pad=(0, 0)):
    ...
    tensor_out = out.view(-1, channel, out_h, out_w)

    if is_video:
        tensor_out = rearrange(tensor_out, '(b f) c h w -> b c f h w', b = b)
